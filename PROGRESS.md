...
## Commerce final build: address collection, webhook rewrite, ship-notify, cart/footer fixes (2026-07-18, iii)
- **Date:** 2026-07-18
- **Branch:** `commerce/order-schema-studio` (same branch throughout this feature) — not merged, not pushed to main. Pushed to remote for a preview deployment at the end of this task (see below).

### 1 — Address collection
New `components/shop/CheckoutAddressModal.tsx`: a right-slide-in modal (same visual pattern as the cart drawer) collecting name/email/phone/multiline shipping address before Razorpay ever opens. Both `BuyControls.tsx` (Buy Now) and `CartDrawer.tsx` (Checkout) now open this modal instead of calling `/api/checkout` directly — one shared component, one validation path, for both entry points. Styled with underline inputs matching `ContactForm.tsx`'s structure (label/span/input, border-bottom only, small-caps label) but in cream-on-cacao to match the drawer it extends; GSAP entrance uses `EASE`/`DUR` from `lib/motion.ts`, no raw `gsap` import. `app/api/checkout/route.ts` now validates all four fields server-side (name, email regex, phone, address) and passes them to `createRazorpayOrder()`, which writes them into the Razorpay order's `notes` as **separate keys** (`customerName`/`customerEmail`/`customerPhone`/`shippingAddress`), not one JSON blob — Razorpay notes cap each value near 256 chars, and a long address must never be able to truncate a sibling field. Item `notes.items` now carries only `{id, slug, qty}` — no pricing — since the webhook re-fetches title/price from Sanity at capture time (see below).
- **Real bug hit and fixed during this task:** the modal initially rendered as a small, badly-positioned box instead of a full-height panel. Root cause: `BuyControls`'s wrapping `.mr-buy--full` div carries a GSAP-set inline `transform` (even the identity matrix), which per the CSS spec makes it the containing block for any `position: fixed` descendant — so the modal's fixed overlay was being positioned relative to that box, not the viewport. Fixed by portaling the modal to `document.body` via `createPortal`, matching the existing pattern already used by `CursorFollower.tsx` for exactly this reason.

### 2 — Webhook rewrite (`app/api/razorpay/webhook/route.ts`)
Full rewrite to the current `order` schema field names (previous version wrote Stripe-era fields that don't exist on the schema at all — see the prior diagnosis entry). Key points:
- **Order notes are read by fetching the order, not from `payment.notes`.** Razorpay does not copy an order's notes onto the payment entity delivered in `payment.captured` — trusting `payment.notes` would have silently reproduced the exact "address never arrives" bug this task exists to fix. The webhook now calls `razorpay.orders.fetch(payment.order_id)` and reads `order.notes` for the customer/address fields. Verified this distinction matters: the signed test payload used to verify this route (below) set `payment.notes: {}` deliberately, and the address still landed correctly.
- **Idempotent on `razorpayOrderId`**, not the payment id — `_id` is derived as `order.<razorpayOrderId>`, checked before creating.
- **`orderNumber`** generated as `MR-<year>-<count+1 padded to 4>` from a new `orderCountQuery` (`count(*[_type == "order"])`) added to `sanity/lib/queries.ts` — all GROQ stays centralized there per project convention.
- **Items are re-fetched from Sanity** (`shopItemsBySlugsQuery`, reused as-is) for `title`/`priceAtPurchase` snapshots — the client/notes never carry pricing, so a tampered or stale note value can't set the recorded price.
- Stock decrement/sold increment logic preserved from the previous version (non-fatal on failure, logged).
- Emails sent via the new `lib/emails.ts` module (below) instead of the old `emails/orderEmails.ts` (left in place — still used by the separate, untouched `app/api/stripe/webhook/route.ts`).

### New `lib/emails.ts`
`sendOrderConfirmation`, `sendOwnerNotification`, `sendShippingUpdate` — cream `#F5EFE4` / cacao `#2C1A0E` typographic HTML, same visual language as the existing enquiry emails. Every send checks the resolved `.error` field and logs it (Resend resolves rather than throws on API-level failures — the same pattern fixed in the enquiry route in the previous phase). `sendOrderConfirmation`/`sendShippingUpdate` fail loudly if `EMAIL_FROM` is unset; `sendOwnerNotification` also requires `MANDAKINI_ORDER_EMAIL`.

### 3 — Ship-notify (`app/api/ship-notify/route.ts`, new)
Sanity webhook target. Auth via `x-webhook-secret` header checked against `SANITY_WEBHOOK_SECRET`. Sends `sendShippingUpdate` and patches `shippedEmailSent: true` only when `status == "shipped"` **and** `awbNumber` is set **and** `shippedEmailSent` is still `false` — that last check runs first, so the webhook's own follow-up patch (which re-fires this same webhook) is a clean no-op instead of a duplicate send.

### 4 — Cart drawer / footer fixes
- **(a) Checkout button:** was functionally invisible — dim amber text/outline on the dark panel, a CSS-cascade-tie loss to `.mr2-cta`'s base rule (same root-cause pattern as the 2026-07-07 hamburger bug). Fixed with a higher-specificity `.mr-cart__foot .mr-cart__checkout` rule: solid terracotta `#B8572A` fill, full width, cream text — the strongest element in the drawer now, by design.
- **(b) Qty stepper vs. Remove:** were 4 flex siblings crammed into one row with 0.5rem gap, reading as "+ REMOVE". Checked git history first per instruction (`git log --all -- components/shop/CartDrawer.tsx`) — confirmed no prior fix ever existed for this. Restructured into `.mr-cart__qty-row` (space-between) containing `.mr-cart__qty` (the stepper, its own tight group) and `.mr-cart__remove` as a separate underlined text link.
- **(c) Stray dot + footer crowding:** the "stray circular dot" was the site's own custom cursor (`.mr2-cursor__dot`, terracotta, z-index 9999) rendering over the drawer — confirmed by moving the mouse and watching it track. Fixed by toggling a `body.mr-modal-open` class (in both `CartDrawer.tsx` and `CheckoutAddressModal.tsx`) that forces the native cursor back on and hides the ring/dot while either panel is open. Needed `!important` in `app/v2.css` — GSAP's `autoAlpha` sets `opacity`/`visibility` via inline style, which beats any class selector regardless of specificity; same technique the existing `cursor: none !important` rule already used. Footer (`.mr-cart__foot`) padding changed to give Checkout/Empty-the-cart breathing room off the bottom edge.
- **(d) Cart pill vs. Say Hello pill:** the floating `.mr-cart__chip` is `position: fixed`; the footer's "Say Hello" pill is normal in-flow content — they only ever collided when the footer scrolled into view. Fixed with an `IntersectionObserver` on `footer.mr2-footer` in `CartDrawer.tsx`: the chip fades out (`.mr-cart__chip--hidden`, transform+opacity transition) whenever the footer is in view, and fades back in once scrolled away. No hard cutoff, no permanent hide.

### Verification (localhost, production build — `npm run build && npm start`, never `next dev`)
- **Address → real Razorpay order:** drove the full flow in the browser (Buy Now → filled the address modal → submitted) and confirmed via Razorpay's own Orders API (`GET /v1/orders`) that the created order's `notes` contained exactly the four separate customer keys plus `items` — not a payment.notes assumption, a real round trip.
- **Webhook, signed self-POST (Razorpay can't reach localhost, so this is the standard workaround):** crafted a `payment.captured` payload referencing that real order, with `payment.notes: {}` (deliberately empty, to prove the fetch-the-order approach), HMAC-signed with a locally-generated `RAZORPAY_WEBHOOK_SECRET`, POSTed directly to `/api/razorpay/webhook`. Result: `200 {received:true}`, and the resulting Sanity document had every field correct — `orderNumber: "MR-2026-0001"`, real `razorpayOrderId`/`razorpayPaymentId`, customer fields and address pulled correctly from the order (not the empty payment notes), items with `title`/`priceAtPurchase` re-fetched from Sanity. Confirmed via `mcp__Sanity__query_documents`, then deleted the test document (unpublish + discard drafts) per project convention.
- **Owner notification email actually sent** to `MANDAKINI_ORDER_EMAIL` and confirmed delivered (Resend's own response, no `.error`). **Confirmation email to the test customer address failed** — logged correctly by the new `.error` check, not silently swallowed — because `EMAIL_FROM` is still Resend's sandbox sender `onboarding@resend.dev`, which Resend only allows sending *to* the account's own verified address. **This is a pre-existing configuration gap, not a bug in this task's code**: until a real domain is verified in Resend and `EMAIL_FROM` is updated to use it, order-confirmation emails to real customers will fail in production exactly the same way. Flagging this explicitly rather than letting it surface later as a "customers aren't getting emails" report.
- **Stock decrement is a no-op for this catalog's items** (`stock: null`, meaning "unlimited/not tracked" per `resolveStock()`) — Sanity's `.dec()` on a null field fails with `Cannot increment "stock" because it is not present`, caught and logged by the existing non-fatal try/catch, not a regression (same behavior the previous webhook version would have hit, had it ever worked).
- **Cart drawer**, desktop (1440px) and mobile (390px), with 1 item and 3 items: solid terracotta Checkout button confirmed strongest element in the panel; qty stepper and Remove link clearly separated; no cursor-dot collision; footer has breathing room at both widths.
- **Footer pill collision**, desktop and mobile, multiple scroll positions: cart chip fades out exactly as the footer (and Say Hello pill) enters view, fades back in on scroll-away — verified at both widths, no overlap frame observed.
- **Not tested:** a real end-to-end Razorpay TEST card payment through the hosted checkout iframe — the iframe opened correctly (confirmed via Razorpay's Test Mode ribbon), but completing a real card payment through cross-origin browser automation wasn't attempted; order-creation and the webhook's processing were instead verified independently (browser-driven order creation + signed self-POST to the webhook), which covers the same code paths without depending on automating Razorpay's own UI.
- `npm run build` clean, zero TypeScript/lint errors beyond pre-existing `<img>` warnings unrelated to this work.
- **Incidental fix:** while adding a local-only `RAZORPAY_WEBHOOK_SECRET` for the signed self-POST test, an `echo >>` append landed on the same line as the existing `SANITY_WEBHOOK_SECRET` value (the file had no trailing newline) and corrupted it. Caught immediately via the resulting 503, and fixed by splitting the two values back onto separate lines in `.env.local` (gitignored, not part of this commit).

### Dashboard setup needed before the webhooks can fire for real
See the separate message accompanying this task's completion for exact Razorpay and Sanity dashboard steps (webhook URLs, event filter, GROQ projection, secret header name) — not repeated here since it depends on the preview deployment URL generated after this push.
...
Investigated three reportedly-broken CTAs before touching anything:
- **Add to Cart works correctly.** `lib/cart.tsx`'s `CartProvider` is real (sessionStorage-backed) and mounted in `app/(site)/layout.tsx`. Live-clicked it — drawer opens with correct item/image/price/subtotal.
- **Buy Now (and the cart drawer's Checkout) is broken by a stock-defaulting bug**, not a missing integration. `components/shop/BuyControls.tsx` → `POST /api/checkout` → `lib/home-data.ts`'s `getPurchasableItems()` (line ~336) does `stock: d.stock ?? 0` — since all 6 real `shopItem` documents have `stock: null` (meaning "not tracked, treat as available"), this resolves to 0 and the route always 409s with `"Only 0 of \"X\" remain."` The *display* logic in the same file (`mapShopDoc`) handles null stock correctly (treats it as 999/plentiful) — the two functions disagree, and the checkout-validation one is wrong. Confirmed live: `POST /api/checkout` → `409`.
- **Send Message silently fails a real Resend error**, previously seen as "Forbidden." `components/contact/ContactForm.tsx` → `POST /api/enquiry` (there's no separate `/api/contact` — it shares the shop's enquiry endpoint). `from`/`to` both default to the same hardcoded `mandakinirao@gmail.com` (no `ENQUIRY_FROM_EMAIL`/`ENQUIRY_NOTIFY_EMAIL` set). Hit Resend's API directly with these exact values and got `403 "The gmail.com domain is not verified."` — confirmed this is the "Forbidden" error. The route's own error handling can never catch this: `resend.emails.send()` resolves (doesn't throw) even on API-level errors, but the code only logs on `Promise.allSettled` rejection, never inspects the resolved `.error` field — so nothing is logged and the client always gets `{ok:true}`. The Sanity **write** does succeed (enquiry is saved), so the message isn't fully lost, but no email ever reaches Mandakini and the visitor is told it worked.
- Also found (flagged, not fixed — out of scope for Phase B): `app/api/razorpay/webhook/route.ts` writes orders using entirely different, pre-Razorpay-rebuild field names (`orderId`, `stripeSessionId`, `shippingAddress` as an object, `items[].shopItemRef`/`price`, `totalAmount`, `paymentStatus`/`fulfillmentStatus`, `orderDate`) that don't match the current `order` schema at all (`orderNumber`, `razorpayPaymentId`, `customerEmail`, `shippingAddress` as text, `items[].shopItem`/`priceAtPurchase`, `amountTotal`, `status`, `createdAt`). Same "GROQ/writer out of sync with a renamed schema" pattern seen twice before in this project (pressItem, testimonial). Real orders created via webhook today would not populate most of the new schema fields below. Needs its own task.

### Phase B — Order schema + Studio structure (executed after approval)
- **`sanity/schemas/order.ts`:** added the missing fields exactly as specified — `razorpayOrderId`, `courierName`, `shippedEmailSent` (boolean, default false), and a `title` snapshot field on each `items[]` entry (alongside the existing `shopItem` ref/`quantity`/`priceAtPurchase`). No existing field renamed. Every field is `readOnly: true` **except** `status`, `awbNumber`, `courierName` — matches "orders are created by webhook only, Mandakini's only job is shipping status." Rewrote `preview.prepare()` to show `orderNumber · customerName · ₹amountTotal · Status`.
- **`sanity.config.ts`:** Orders (still pinned in the Commerce tier, same position) now opens a grouped sub-list — Paid, Shipped, Delivered & Cancelled, All Orders — each newest-first (`createdAt desc`), built via a small `orderGroup()` helper wrapping `S.documentList().schemaType('order').apiVersion('2024-01-01').filter(...)`. The `apiVersion` call matters: without it Sanity logs a console warning per custom-filtered list ("this will be required in the future") — added to keep Studio warning-free.
- **No GROQ changes** — no order-reading UI exists yet (Phase B was schema/Studio only), so `sanity/lib/queries.ts` untouched.
- **Verified end-to-end in Studio** (not just schema-read): created one temporary test order via Sanity MCP (`orderNumber: MR-TEST-001`, `status: paid`), confirmed the preview line rendered exactly as designed, confirmed a read-only field (Order Number) visually accepts focus but silently rejects typed input, then edited `status` (Paid → Shipped) and typed into `awbNumber` — both accepted, saved, and the document **live-moved** from the Paid group to the Shipped group in the sidebar without a refresh (confirms the grouped filters are reactive, not a one-time snapshot). Discarded the test draft immediately after — 0 order documents remain in the dataset.
- **Build:** `npm run build` clean, zero TypeScript errors (`npx tsc --noEmit`), `/studio/structure` loads with no schema-load errors (confirmed via console — only pre-fix apiVersion warnings, gone after the fix).
- **Not pushed anywhere** — branch `commerce/order-schema-studio` is local-only, per instruction to never push to production.

## Journal article editorial polish — pull quotes, captions, richer ending (2026-07-09)
- **Date:** 2026-07-09
- **Branch:** `feat/journal` — still not merged to main.
- Client feedback after reviewing the two test articles: keep both (to show Mandakini), add more content, and the article page "looks very bland" — asked to research how other editorial sites structure an article and match that (explicitly: creative-professional sites generally, not artist-specific blogs).
- **Research:** fetched a live Kinfolk.com article (`/stories/david-chipperfield/`) as a concrete reference. Diagnosis: the layout system (left/right/top/bottom, collage, carousel) was already structurally sound — what was missing was typographic variety within the plain paragraph flow: no lede treatment on the opening paragraph, no way to set off a striking line as a pull quote, no visible image captions (only invisible alt text), and a flat text-only prev/next ending instead of a richer close.
- **Schema** (`sanity/schemas/journalSection.ts`): added optional `caption` (string) to each image in a paragraph's `images` array, alongside the existing required `alt` — alt stays accessibility-only, caption is the visible italic line under the photo. Added optional `pullQuote` (boolean, hidden unless the paragraph has zero images) — when set, the paragraph renders as a large centered italic pull quote instead of body text.
- **Rendering**: `JournalSection.tsx` branches to a `<blockquote className="mr-journal__pullquote">` when `pullQuote` is set (bypasses the normal left/right/top/bottom grid entirely — a pull quote is always centered). Image groups now render inside a `.mr-journal__image-group` wrapper so a caption can sit directly under the photo/collage/carousel as a sibling (required restructuring the CSS `order` rules, which previously targeted the image/collage/carousel classes directly as grid children). New CSS: `.mr-journal__pullquote` (large serif italic, thin toffee rules above/below), `.mr-journal__caption` (small italic muted), and an automatic lede rule (`.mr-journal__section:first-of-type .mr-journal__text p:first-child`) that gives the article's opening paragraph slightly more weight — no schema field needed, standard editorial convention applied automatically.
- **Richer ending**: `JournalArticle.tsx`'s prev/next nav replaced with a "More from the Journal" block reusing the listing page's card visual language at a smaller scale (`.mr-journal-card--mini`). Deduplicates when prev/next resolve to the same post (inevitable with only 2 total posts — confirmed this was a real bug caught before shipping: `[prev, next]` both wrap to the same single "other" post whenever there are exactly 2 posts, which would have produced a duplicate React key and a repeated card) and excludes the current post itself.
- **Content**: patched both existing test posts via the Sanity MCP (no new uploads) — added a pull quote, at least one image caption, and a closing paragraph to each, so they read as complete short pieces rather than a bare checklist of layout combinations.
- **Verified**: `npm run build` clean; both articles reviewed end-to-end on localhost (production mode) — lede paragraph, pull quote, captions (single-image and collage), and the deduplicated "More from the Journal" ending all confirmed rendering correctly; `/?v=1` confirmed untouched (`git diff --stat` shows no changes under `components/home/`).
